...
{
	if ((last == NULL || token_is(last, TOKEN_OPENING)) && token_is(current, TOKEN_OPERATOR))
		return (1);
	if (token_is_any(last, TOKEN_OPERATOR | TOKEN_REDIRECTION) && token_is(current, TOKEN_OPERATOR))
		return (1);
	return (0);
}
