	...
	do
		...
		if [[ $line == \#* ]] || [[ $line == "" ]] ; then
			...
			do
				...
				minishell_output=$(echo -n "$INPUT" | $MINISHELL_PATH/$EXECUTABLE 2>tmp_err_minishell)
				exit_minishell=$?

				bash_output=$(echo -n "enable -n .$NL$INPUT" | bash 2>tmp_err_bash)
				exit_bash=$?
